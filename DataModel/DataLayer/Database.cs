...
namespace DataModel.DL
{
    public class Database : SQLiteConnection
    {
        ...
        public Database(string path) : base(path, true, "Password")
        {
            try
            {
                LogsHandler.WriteDebug(path);

                LogsHandler.WriteDebug("DataModel.DL.Database: CreateTable MainTable");

                // create the tables
                CreateTable<Models.Tables.MainTable>();
            }
            catch (Exception Ex)
            {
